...
//Diff CIs
  ...
  CIs_avg = CIs_avg.map(function(e) {return githubCIs + "ci_" + e}); // add URL path
  ...
  CIs_diff = CIs_diff.map(function(e) {return githubCIs + "ci_" + e}); // add URL path
  preloadimages.push(CIs_diff);
  CIs_diff = CIs_diff.map(function(e) {return {imgHtml: "<img class='imgJudge' src='" + e + "'>"}});
  CIs_diff = _.shuffle(CIs_diff); // randomize order
  ...
  var previewCIinst_avg = {
    ...
    stimulus: function() {
      ...
      html += "<p class='justify'>In this study, you task is to <b>evaluate 4 faces</b>. You will indicate, "
      ...
      html += "<b>You will start by evaluating 2 of the 4 faces</b>. Before the evaluation task, <b>we will briefly display these 2 faces</b> ";
...
// Judgments for condition-level average CIs
  ...
  CIs_diff.map(function(ciImg){
    ...
    judgments_diff.push(likert_trial);
  });
  ...
  timeline.push(previewCIinst_diff);
  timeline.push(previewCI_diff);
  timeline.push(judgments_diff);  
  timeline.push(age);
  ...
  jsPsych.init({
    ...
    on_finish: function(data) { 
      ...
      database.ref("eval_faces_Black_AAT/" + id + "/").update({subdata}).then(function() { 
